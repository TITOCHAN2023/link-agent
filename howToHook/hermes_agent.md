# AgentLink → Hermes Webhook 无损推送指南

## 问题

AgentLink bridge 收到 P2P 消息后，agent 不知道。需要让消息**主动唤醒 agent**。
...
Bridge (:7654) 收到消息
    ├── 自动写入 ~/.agentlink/{roomId}/inbox.jsonl (JSONL, source of truth)
...
>
> **inbox 路径**：`~/.agentlink/{roomId}/inbox.jsonl` — JSONL 格式（每行一个 JSON），bridge 自动维护，webhook 脚本直接读取，不需要额外的存档文件。
...
`/tmp/agentlink_webhook.sh`：

```bash
#!/bin/bash
WEBHOOK_URL="http://127.0.0.1:8644/webhooks/agentlink-msg"
SECRET="你的HMAC密钥"
FROM="${1:-unknown}"
TYPE="${2:-chat}"
ID="${3:-}"
...
# 路径: ~/.agentlink/{roomId}/inbox.jsonl（JSONL 格式，一行一条 JSON）
INBOX="${HOME}/.agentlink/${ROOM_ID}/inbox.jsonl"
...
> **inbox 路径**：bridge 自动将所有收到的消息写入 `~/.agentlink/{roomId}/inbox.jsonl`（JSONL 格式）。webhook 脚本直接读这个文件，不需要维护单独的存档。

```bash
chmod +x /tmp/agentlink_webhook.sh
```

### 4. 启动 Bridge（接上 hook）

```bash
cd /tmp/AgentLink && node bin/agentlink.js bridge --port 7654 \
  --name TitoClaw --perm helper \
  --on-message 'bash /tmp/agentlink_webhook.sh {from} {type} {id} {roomId} {agentId}' \
  --on-connect 'echo "$(date) 🔗 [{roomId}] {peer} connected ({permission})" >> /tmp/agentlink_live.log' \
  --on-disconnect 'echo "$(date) ❌ [{roomId}] disconnected: {reason}" >> /tmp/agentlink_live.log' \
  --foreground &
```
...
```bash
# 用法: bash /tmp/agentlink_receiver.sh <roomId>
...
| `/tmp/agentlink_webhook.sh` | bridge hook → 签名 webhook 调用（带 roomId） |
| `/tmp/agentlink_receiver.sh` | （可选）按房间 long-poll 备份通道 |
| `/tmp/agentlink_live.log` | 人类可读的消息日志（含房间标识） |
| `~/.agentlink/{roomId}/inbox.jsonl` | bridge 自动维护的消息存档（JSONL，source of truth） |
...
- ✅ Subscription 已创建（`agentlink-msg` → telegram）
- ✅ Bridge 运行中，hook 已接上
