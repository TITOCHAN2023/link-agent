...
function makeClient(opts, extra = {}) {
  ...
  return opts.json ? new AgentJSON(args) : new AgentClient(args);
}
...
program
  ...
  .option('-n, --name <name>', 'Your Claw name', rc.name || 'Agent')
...
const bridgeCmd = program
  ...
  .action(async (opts) => {
    const port = parseInt(opts.port, 10);
    const iceRaw = opts.ice || process.env.AGENTLINK_ICE_SERVERS || null;
    ...
    if (opts.daemonChild) {
      ...
      const bridge = new AgentBridge(bridgeOpts);
      await bridge.start();
...
bridgeCmd
  ...
  .option('--agent <id>', 'Agent identity (or set AGENTLINK_AGENT_ID env var)')
  .action((roomId, opts) => {
    const port = parseInt(opts.port, 10);
    const agentId = opts.agent || process.env.AGENTLINK_AGENT_ID || null;
    const body = {};
...
STEP 3 — Create a new room OR join an existing one:
  ...
  SET ONCE WITH ENV VAR (so you don't need --agent every time):
    export AGENTLINK_AGENT_ID=claude-a
...
================================================================
  ...
  7. Multiple agents on one machine? Set AGENTLINK_AGENT_ID env var or use
